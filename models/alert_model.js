...
function normalizeCondition(cond) {
  ...
  if (keys.length !== 1)
    throw new Error("Condition must have exactly one field");
  ...
  if (opKeys.length !== 1)
    throw new Error("Operator object must have exactly one operator");
...
alert_model_Schema.statics.convertToJSONFormat = function (input) {
  ...
  if (keys.length === 1 && (keys[0] === "$and" || keys[0] === "$or")) {
    ...
    if (!Array.isArray(arr) || arr.length === 0)
      throw new Error("Logical operator must wrap a non-empty array");
