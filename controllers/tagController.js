...
const syncTags = async (req, res) => {
  try {
    ...
    if (!Array.isArray(originalIds) || !Array.isArray(newNames))
      return res
        .status(400)
        .json({ message: "originalIds and newNames must be arrays" });

    // Normalize names
    const cleaned = newNames.map((n) => (n && String(n).trim()) || "");
    if (cleaned.some((n) => n === ""))
      return res.status(400).json({ message: "Tag names cannot be empty" });
      ...
      for (const idToDelete of deletions) {
        ...
        try {
          await Alert.updateMany(
            { tags: idToDelete },
            { $pull: { tags: new mongoose.Types.ObjectId(idToDelete) } },
          );
    ...
    const nameToId = {};
    allTags.forEach((t) => {
      nameToId[t.name.toLowerCase()] = String(t._id);
    });
    ...
    for (let i = 0; i < maxLen; i++) {
      ...
      const requestedColor =
        colors && colors[i] ? String(colors[i]).trim() : null;

      // If this ID was just deleted, do NOT process it in the loop
